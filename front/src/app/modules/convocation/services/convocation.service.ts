...
import { CurrentReponse, RemplcamentDetail } from '../model/convocation.model';
...
export class ConvocationService {
  ...
  genrateInfoMandat (idSeanceConvocation  : number , idMembreRempl : number ) : Observable<ApiResponse> {
    let httpparam = new HttpParams()
    .set("idSeanceConvocation",JSON.stringify(idSeanceConvocation)).set("idMembreRempl",JSON.stringify(idMembreRempl))
    return this.http.get<ApiResponse>(environment.BASE_URL + ApiConstant.CONVOCATION_PREFIX + 'genrateInfoMandat' , {params:httpparam});
  }
}
